...
class ZSDataset(Dataset):
    ...
    def mark_sem_entity(self, e1_span, e2_span, sentence_token_ids, sent_len, start_of_sentence_tokens, sentence_tokens_positions):
        e1_start = -1
        e2_start = -1
        ...
        for i in range(0, len(sentence_tokens_positions)):
            if sentence_tokens_positions[i][1] == e1_span[1]:
                e1_end = start_of_sentence_tokens + i  # match the last token in the span
            elif sentence_tokens_positions[i][1] == e2_span[1]:
                e2_end = start_of_sentence_tokens + i  # match the last token in the span
    ...
    def __getitem__(self, idx):
        ...
        segments_tensor = torch.tensor([0] * (1 + len(tokens) + 1) +
                                       [1] * (len(tokenized_relation_desc) + 1), dtype=torch.long)
        start_of_sentence_tokens = len(
            ["[CLS]"] + tokenized_relation_desc + ["[SEP]"])
        marked_e1, marked_e2 = self.mark_sem_entity(
            e1_span, e2_span, tokens_ids, len(segments_tensor), start_of_sentence_tokens, sentence_tokens_positions)
...
def create_mini_batch_fewrel_aio(samples):
    ...
    masks_tensors = masks_tensors.masked_fill(
        tokens_tensors != 0, 1)
    return tokens_tensors, segments_tensors, marked_e1, marked_e2, masks_tensors, label_ids
